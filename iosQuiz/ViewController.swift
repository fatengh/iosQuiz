...
class ViewController: UIViewController {
    ...
    @IBOutlet weak var qustionLebel: UILabel!
    
    @IBOutlet weak var answerLebel: UILabel!
    
    @IBAction func answerButtonPrees(_ sender: UIButton) {
        // if answerLabel is not hidden, then make it hidden
        if !answerLebel.isHidden{
            answerLebel.isHidden = true
            // else set it not hidden
        }else{
            answerLebel.isHidden = false
        }
    }
    ...
    func updateUI(){
        ...
        qustionLebel.text = quizBank[currentQuestion].0
        // set answerLabel's text to equal quizBank[currentQuestion].1
        answerLebel.text = quizBank[currentQuestion].1
